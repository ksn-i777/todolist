...
import { TaskStatus, TaskType } from './tasks-api'
  ...
  return (
    <div style={props.task.status === TaskStatus.Completed ? styleDoneTask : styleTask}>
      <Checkbox
        ...
        onChange={(e) => onUpdateTaskStatus(props.task.id, e)}
      />
      <EditableSpan
        disabled={props.taskEntityStatus === 'loading'}
        spanTitle={props.task.title}
        changeSpanTitle={(newSpanTitle: string) => { onUpdateTaskTitle(props.task.id, newSpanTitle) }}
      />
      <IconButton
        disabled={props.taskEntityStatus === 'loading'}
        aria-label="delete"
